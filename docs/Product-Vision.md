...
**Agents:** Fill-in blanks. If context is thin, re-open [`agent/workflow/product-vision.md`](templates/agent/workflow/product-vision.md). Lock gate: [`agent/workflow/understanding.md`](templates/agent/workflow/understanding.md) §4. Under **ship-first**, `draft` is destination-only — do **not** wait for confirm before coding. *Lock product shape* starts the confirm gate.
