...
| **Model watch** | New xAI SKU / resolution on public docs | `uv run python scripts/watch_xai_models.py` | No API key. Diffs models/pricing/Imagine pages vs `scripts/data/xai_models_watch.json`. Release notes are not scanned. Daily Actions workflow `.github/workflows/watch-xai-models.yml` opens one `xai-models` issue for slugs and resolutions that are not already listed on an open issue (the New slugs / New resolutions lines, the title, or a comment). Tokens already listed stay quiet. After you update the kit, `--write-baseline` and close the issue. |
...
- PyPI publish: bump `pyproject.toml` version (and the `PackageNotFoundError` fallback in `src/xaikit/__init__.py`), merge to master, then tag `v<that-version>` (next `v0.1.1`; `v0.1.0` is already published). Never reuse a version. Trusted Publishing — no tokens in the repo. Do **not** dual-write a Human-TODO tag/`procure` row for that (standing).
