...
- PyPI publish: bump `pyproject.toml` version (and the `PackageNotFoundError` fallback in `src/xaikit/__init__.py`), merge to master, then tag `v<that-version>` (currently `v0.1.0a6`). Never reuse a version. Trusted Publishing — no tokens in the repo. Do **not** dual-write a Human-TODO tag/`procure` row for that (standing).
