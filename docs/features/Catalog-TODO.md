...
**Blocked by:** —  
**Last session:** 2026-09-23 — Grok 4.7 is the public chat/code default. `BOOTSTRAP_MODEL` and the under-200k price row are `grok-4.7` ($2 / $6). Reasoning and chat extras stay the 4.6+ family. Batch stays off (model page: not supported). Watch baseline also records docs-path `grok-4-7` and `grok-voice-transcribe-1.0` / `2.0` (same STT rates; meter key stays `stt`).
