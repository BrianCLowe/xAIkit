...
- **Orchestrator git:** `orchestrator.git.mode` — recommend **`milestone-pr`** (overnight: per-milestone PR — several related TODOs + concurrent implementers when they do not overlap → squash before ready → CI/Bugbot → merge → next branch), or **`branch-pr-squash`** (one morning PR, no merge), **`branch-pr`**, **`branch-push`**, **`local`**, **`current-push`** (never silent-default), or **`none`**. Asked if unset even under sync `auto-all`. **Cloud Agent** orchestration this-runs **`milestone-pr`** when durable mode is local-oriented or `branch-pr*` (does not rewrite settings). See [orchestrator-git](templates/agent/roles/orchestrator-git.md).
