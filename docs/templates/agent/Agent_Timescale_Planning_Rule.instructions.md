...
**Lock shape early (modular docs):** Product-defining surface/architecture identity belongs in Understanding **What this is / is NOT** when the project uses Understanding (`docs_profile: prevent` / balanced when ambiguous / *lock shape*) — e.g. one continuous surface vs N editors. **Assumptions = real forks only** (Workflow §4 lock gate). Do **not** use Assumptions to offer an MVP / half-measure that finishes faster — that is this rule, not a lesser-path ask. Under **build-first**, put identity constraints on the **spec** (overview / Decisions) until an Understanding exists. Module/API detail stays on the **spec**. Once shape is locked, plans and TODOs must target it. When `docs/Product-Vision.md` is **confirmed**, that whole-product end-state is the target the stems must fit (Workflow §4.5) — do not ship a pile of locally-correct features that fight the picture.
...
**With this pack’s modular docs:** If TODO / Current focus encodes an interim that fights confirmed Understanding, rewrite the TODO toward the target **before** implementing. “Tight scope” means do not wander into unrelated work or repo-wide audits — **not** “ship human-sized half-architectures,” **not** “domain checklist complete = operable product,” and **not** “all TODOs `[x]` while operable Acceptance for the claim stays open” (Workflow §5.3). **Finished kit:** when the product contract is knowable, spec **that** destination and put covering TODOs on existing stems (Workflow §5.4) — do not omit the backlog because you could write the spec, do not wait for a human to “pick up” each leftover, and do not treat a terse wrap-the-public-API goal as a stub that needs hand-holding. Drain is many verified units (not one mega-commit). New map rows only when a slice is actually next — never for vague planned-only items (Workflow §0 inventory).
