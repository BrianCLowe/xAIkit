...
- **Document Map = files on disk.** Adding a §3.1/§3.2 row requires creating the **profile default file set** in the same turn (always spec + core TODO; Understanding per §0.1). A **Catalog** map cell requires `*-Catalog.md` the same turn. Never leave map-only “planned” rows. Bootstrap Step 3d. **Do not add filler §3.1 rows.**
