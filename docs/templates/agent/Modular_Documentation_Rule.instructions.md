...
- Update **Current focus** + `-TODO.md` (`[x]` + date; **move** a finished plotted item into **Completed**). Do not add a Completed row for an incidental fix — git already has it (Workflow §5). Human-TODO feedback → sync owner + Human-TODO Done (§13); never mark human rows from assumptions. A playtest report that the scenario did not hold reopens that outcome (§5.5).
- **Work-verifier** after a claimed unit, before mark-done: the claimed TODO item against that unit’s diff. Spawn it when the adapter exists; otherwise follow `docs/templates/agent/roles/work-verifier.md` in this session. Declining doc-roles does not skip it. Older checked rows are the warden overclaim reopen, not this compare.
