...
| **Orchestrator** | [`orchestrator.md`](orchestrator.md) + [`orchestrator-git.md`](orchestrator-git.md) | **Parent-only** loop + git delivery (**`milestone-pr`:** per-slice PR → CI/Bugbot → merge → next branch; **`branch-pr*`:** build-verify → todo-warden → squash? → ready → **return to default** if run created the branch) | Agent work cleared / hard-blocked / budget — report + human verify inbox |
