> **Workflow module.** Open only when `docs/ADT-settings.yaml` → `team_inbox.enabled` is true. Unset / `enabled: false` = human-only inbox — do not open this file. Solo dual-write stays in [`human-todo.md`](human-todo.md).
