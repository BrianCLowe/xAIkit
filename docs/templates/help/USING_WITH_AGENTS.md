...
| **Product vision** | Whole-product end-state picture. **Always create** (lightweight on `balanced` / `ship-first`). **`ship-first`:** not a gate until *lock product shape*. Confirmed vision: features must not fight it | [`Product_Vision_Template.md`](../Product_Vision_Template.md) · [§4.5](../agent/workflow/product-vision.md) |
