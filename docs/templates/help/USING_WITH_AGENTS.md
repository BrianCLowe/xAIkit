...
| **Orchestrator git** | `local` · `branch-pr` · `branch-pr-squash` *(recommend + forge)* · `branch-push` · `current-push` · `none` — ask if unset; forge probe on pick; **Cloud Agent** this-runs squash-PR if durable is local-oriented | Step 3p / B0.6 · [`orchestrator-git.md`](../agent/roles/orchestrator-git.md) |
