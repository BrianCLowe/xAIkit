...
async def connect_realtime_websocket_async(uri: str, **kwargs: Any) -> Any:
...
class AsyncRealtimeSession:
    """One duplex realtime-voice connection (async, async-context-manager)."""
    ...
    async def send_audio(self, audio: bytes | str, *, commit: bool = False) -> None:
        """Append inbound audio (``input_audio_buffer.append``)."""
        payload = {
            "type": "input_audio_buffer.append",
            "audio": _b64_audio(audio),
    ...
    async def recv(self, *, timeout: float | None = None) -> dict[str, Any] | bytes:
        """Receive the next server event (JSON dict) or binary audio frame."""
        try:
            ...
            if _is_normal_close(exc):
                raise RealtimeClosed(str(exc) or "Realtime connection closed") from exc
            await self._fail(exc, "Realtime recv failed")
        if isinstance(message, bytes):
            return message
        if isinstance(message, dict):
            return message
        try:
            ...
            await self._fail(exc, "Realtime recv returned non-JSON")
        if not isinstance(payload, dict):
            raise RuntimeError("Realtime recv returned unexpected payload")
        return payload

    async def events(self) -> AsyncIterator[dict[str, Any] | bytes]:
        """Yield server events until the socket closes (normal close is success)."""
        while True:
            try:
    ...
    async def _send_json(self, event: dict[str, Any]) -> None:
        try:
            ...
            await self._fail(exc, "Realtime send failed")

    async def _fail(self, exc: BaseException, prefix: str) -> None:
        err = self._error_class(exc)
        duration = max(0.0, time.monotonic() - self._t0)
        self._finish_meter(success=False, error=err, duration=duration)
        if not self._closed:
